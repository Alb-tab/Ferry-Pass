...
import { allAsync, runAsync } from '../models/database.js';
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await runAsync(
      'INSERT INTO routes (name, origin, destination, duration_minutes) VALUES (?, ?, ?, ?)',
      [name, origin, destination, duration_minutes || 0]
    );

    res.status(201).json({ id: result.id, name, origin, destination, duration_minutes });
  } catch (error) {
    console.error(error);
